...
interface DataTableProps<TData, TValue> {
    ...
    setExportFn: (fn: (rows: { [k: string]: any; }[]) => void) => any,
}
...
export function DataTable<TData, TValue>({
    ...
    const totalGoats = table.getFilteredRowModel().rows.reduce((sum, row) => {
        const male = hasColumn("maleGoats") ? parseInt(row.original.maleGoats) || 0 : 0;
        const female = hasColumn("femaleGoats") ? parseInt(row.original.femaleGoats) || 0 : 0;
        return sum + male + female;
    }, 0);
    
    const filename = document.title + `- ${new Date().toISOString().replace(/[:.]/g, '-')}`;
            ...
            <Card padding="none" style={{
                ...
                <Stack justifyContent="center" alignItems="center" style={
                    ...
                    <div className="items-center gap-3 hidden lg:flex">
                        ...
                        <Select hideLabel className="hidden lg:flex"
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <ButtonGroup className="hidden lg:flex">
                            <Button variant="primary" aria-describedby="first" onClick={() => table.setPageIndex(0)}
                                disabled={!table.getCanPreviousPage()}>
                                <PiCaretDoubleLeftBold className={`h-4 w-4 ${table.getCanPreviousPage() ? 'text-white' : ''}`} />
                            </Button>
                            <Tooltip id="first">First page</Tooltip>
                            <Button variant="primary" aria-describedby="previous" onClick={() => table.previousPage()}
                                disabled={!table.getCanPreviousPage()}>
                                <Icon name="arrow-left" />
                            </Button>
                            <Tooltip id="previous">Previous</Tooltip>
                            <Button variant="primary" aria-describedby="next" onClick={() => table.nextPage()}
                                disabled={!table.getCanNextPage()}>
                                <Icon name="arrow-right" />
                            </Button>
                            <Tooltip id="next">Next</Tooltip>
                            <Button variant="primary" aria-describedby="last" onClick={() => table.setPageIndex(table.getPageCount() - 1)}
                                disabled={!table.getCanNextPage()}>
...
function Filter({ table, header }: FilterProps) {
    ...
    //     <Select hideLabel style={{ "--n-input-inline-size": `${800 / table.getHeaderGroups().length} px ` } as React.CSSProperties}
    //         onInput={(e) => {
